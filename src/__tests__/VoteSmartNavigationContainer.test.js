...
describe ('<VoteSmartNavigationContainer>', () => {
  ...
  test('should have "Local" navlink', () => {
    const { getByText } = render(<Router initialEntries={['/local']}><VoteSmartNavigationContainer /></Router>)
    const button = getByText('Local')
    expect(button.className).toMatch(/active/)
  })

  test('should have "Federal" navlink', () => {
    const { getByText } = render(<Router initialEntries={['/federal']}><VoteSmartNavigationContainer /></Router>)
